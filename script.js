...
(function () {
  ...
  function updateGreeting() {
    ...
    greetingEl.innerHTML = `<i class="fas fa-${hour < 12 ? "sun" : hour < 17 ? "sun" : "moon"}"></i> ${greet}`;
  }
  updateGreeting();
  ...
  function setBackgroundByCondition(condition) {
    const bgMap = {
      Clear:
        "url('https://images.unsplash.com/photo-1545569341-973eb8501872?q=80&w=2070&auto=format&fit=crop')",
      Clouds:
      ...
      Smoke:
        "url('https://images.unsplash.com/photo-1504630085234-6e0d1aa4a6d8?q=80&w=1974&auto=format&fit=crop')",
      Dust: "url('https://images.unsplash.com/photo-1507238691740-187a5b1d37b8?q=80&w=2010&auto=format&fit=crop')",
    };
    const fallback =
      "url('https://images.unsplash.com/photo-1504608524841-42fe6f032b4b?q=80&w=1965&auto=format&fit=crop')";
    document.body.style.backgroundImage =
      bgMap[condition] || bgMap["Clear"] || fallback;
  }
  ...
  function renderWeather(data) {
    ...
    const dewPoint =
      (data.main.temp - (100 - data.main.humidity) / 5).toFixed(1) +
      (isMetric ? "°C" : "°F");
    const windDir = data.wind.deg ? data.wind.deg + "°" : "--";
    const gust = data.wind.gust
      ? isMetric
        ? data.wind.gust.toFixed(1) + " m/s"
        : (data.wind.gust * 2.23694).toFixed(1) + " mph"
      : "--";
    ...
    document.querySelectorAll(".detail-item").forEach((item) => {
      item.addEventListener("click", (e) => {
        ...
        if (metric === "humidity") extra = { dewPoint };
          ...
          extra = { seaLevel: data.main.pressure, trend: "steady" };
